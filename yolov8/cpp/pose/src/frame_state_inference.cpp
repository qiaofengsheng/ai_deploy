...
int FrameStateInference::loadModel(const std::string &modelPath, string configPath)
{
    ...
    std::vector<std::vector<int>> outputSizes = {{1, this->anchors_, this->num_poses*3+this->num_classes+4}};
...
void FrameStateInference::postprocess(std::vector<Instance> instances,std::string &resultString){
    ...
    for (auto obj : instances)
    {
        ...
        for(int m=0;m<obj.poses.size();++m){
            tmp_pose_data["x"]=int(obj.poses[m].x);
            tmp_pose_data["y"]=int(obj.poses[m].y);
            tmp_pose_data["conf"]=obj.poses_prob[m];
            tmp_data["poses"].append(tmp_pose_data);
        }
...
int FrameStateInference::detection(cv::Mat &inputImage, std::string &outputs)
{
    ...
    float *tmp_output = new float[maxBatchSize_ * this->anchors_*(this->num_poses*3+this->num_classes+4)];
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        std::vector<cv::Point> tmpPoses;
        for(auto tmpP:instances[i].poses){
            float tmpPx = (tmpP.x-add_w)/scale;
            float tmpPy = (tmpP.y-add_h)/scale;
            tmpPx = std::max(std::min(tmpPx, (float)(img_w - 1)), 0.f);
            tmpPy = std::max(std::min(tmpPy, (float)(img_h - 1)), 0.f);
            tmpPoses.push_back(cv::Point(tmpPx,tmpPy));
        }
        instances[i].poses = tmpPoses;
    }
...
void FrameStateInference::generateDetInstances(float *feat_blob, float prob_threshold, std::vector<Instance> &objects)
{
    ...
    {
        int basic_pos = anchor_idx * (this->num_poses*3+num_classes + 4);
        ...
        if(box_prob>this->conf_thr_){
            std::vector<cv::Point> tmpPoses;
            std::vector<float> tmpPosesProb;
            for(int k=0;k<this->num_poses;++k){
                // std::cout<<feat_blob[basic_pos+4+this->num_classes+3*k]<<std::endl;
                // std::cout<<cv::Point(feat_blob[basic_pos+4+this->num_classes+3*k+1],feat_blob[basic_pos+4+this->num_classes+3*k+2])<<std::endl;
                std::cout<<feat_blob[basic_pos+4+this->num_classes+3*k+0]<<" "<<feat_blob[basic_pos+4+this->num_classes+3*k+1]<<" "<<feat_blob[basic_pos+4+this->num_classes+3*k+2]<<std::endl;
                tmpPosesProb.push_back(feat_blob[basic_pos+4+this->num_classes+3*k+2]);
                tmpPoses.push_back(cv::Point(feat_blob[basic_pos+4+this->num_classes+3*k],feat_blob[basic_pos+4+this->num_classes+3*k+1]));
            }
            Instance obj;
            obj.rect.x = x0;
            obj.rect.y = y0;
