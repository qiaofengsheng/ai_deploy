...
int main(int argc, char const *argv[])
{
    ...
    std::string onnxPath = "/home/qfs/project/server_project/study/tensorrt/ai_deploy/yolov8/cpp/pose/model/yolov8n-pose.trt";
    std::string configPath = "/home/qfs/project/server_project/study/tensorrt/ai_deploy/yolov8/cpp/pose/model/config.ini";
    ...
    if(reader.parse(outputs, root)){
        for(auto data:root["data"]){
            ...
            for(auto k:data["poses"]){
                // std::cout<<cv::Point(k["x"].asInt(),k["y"].asInt())<<std::endl;
                cv::circle(matImg,cv::Point(k["x"].asInt(),k["y"].asInt()),3,colors[cls_index],-1);
            }
        }
