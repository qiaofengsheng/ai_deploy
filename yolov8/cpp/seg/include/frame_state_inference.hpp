...
public:
    ...
    virtual int detection(cv::Mat &inputImage, std::vector<outStruct> &outputs);
...
private:
    ...
    cv::Mat maskProcess(Instance instance,int add_w,int add_h,float scale,cv::Size sourceSize);
    void generate_grids_and_stride(std::vector<int> &strides, std::vector<GridAndStride> &grid_strides);
    void generateDetInstances(float *feat_blob, float *feat_blob2, float prob_threshold, std::vector<Instance> &objects);
    ...
    void postprocess(std::vector<Instance> instances,std::vector<outStruct> &resultString);
