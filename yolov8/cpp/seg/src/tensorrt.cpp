                        ...
                        std::vector<std::vector<int>> inputSizes, std::vector<std::vector<int>> outputSizes, int maxBatchSize)
{
    ...
    CHECK(cudaMalloc(&vecBuffer_[2], maxBatch * outputVolumSize[1] * sizeof(float)));
    // Create stream
...
int TensorRT::doBatchInference(std::vector<float *> input, const int batch_size, std::vector<float *> &outputs)
{
    ...
    CHECK(cudaMemcpyAsync(outputs[1], vecBuffer_[2], maxBatch * outputVolumSize[1] * sizeof(float), cudaMemcpyDeviceToHost, stream_));
    cudaStreamSynchronize(stream_);
    return 0;
}
